...
func HostSpecs() []artifact.ToolSpec {
	...
	return []artifact.ToolSpec{
		...
		spec("office_create", "Create a real .docx, .xlsx, or .pptx under the workspace. Spreadsheets accept formula cells starting with =.", "write_workspace", obj(map[string]any{"path": strP(), "kind": strP(), "title": strP(), "body": strP(), "headings": map[string]any{"type": "array", "items": strP()}, "rows": map[string]any{"type": "array"}, "slides": map[string]any{"type": "array", "items": strP()}}, "path"), write),
		spec("office_edit", "Replace text inside an office document under the workspace.", "write_workspace", obj(map[string]any{"path": strP(), "old_str": strP(), "new_str": strP()}, "path", "old_str", "new_str"), write),
		...
		spec("schedule_create", "Create a cron, once, or heartbeat job. Jobs run in an isolated worktree and never send_as_you without a later approval.", "schedule", obj(map[string]any{"kind": strP(), "spec": strP(), "prompt": strP()}, "prompt"), write),
		...
		spec("browser_open", "Open a URL in the isolated browser profile (not the operator Chrome).", "browser", obj(map[string]any{"url": strP()}, "url"), net),
		...
		spec("browser_download", "Download a URL into the workspace via the isolated browser.", "browser", obj(map[string]any{"url": strP(), "path": strP()}, "url", "path"), net),
		spec("clipboard_read", "Read the OS clipboard into the turn. Contents are untrusted.", "", obj(map[string]any{}), read),
		spec("clipboard_write", "Write text to the OS clipboard.", "write_workspace", obj(map[string]any{"text": strP()}, "text"), write),
		spec("screenshot_region", "Capture the primary display into a workspace PNG.", "read_workspace", obj(map[string]any{"path": strP()}), read),
		...
		spec("connector_draft", "Create a send_as_you draft. Sending is a separate approval.", "write_connector", obj(map[string]any{"account": strP(), "to": strP(), "subject": strP(), "body": strP()}, "account", "body"), write),
		spec("connector_send", "Send a previously drafted message as the operator.", "send_as_you", obj(map[string]any{"draft_id": strP()}, "draft_id"), Annotations{Exclusive: true, OpenWorld: true, Destructive: true}),
