...
func HostSpecs() []artifact.ToolSpec {
	...
	return []artifact.ToolSpec{
		...
		spec("wait", "Pause up to 30 seconds. Not for hung HTTP clients — use web_fetch/web_search instead of wait/tasklist.", "", obj(map[string]any{"seconds": intP()}), read),
		...
		spec("web_fetch", "HTTP GET a public http(s) URL and return extracted text. Atom/RSS feeds are compacted to id/title/date/summary. If https returns 406, the tool retries http. Never used for localhost or private IPs. Requires network approval.", "network", obj(map[string]any{"url": strP()}, "url"), net),
