...
(function () {
  ...
  var BODY_GEN = {
    ...
    /** 叠盘：两个偏心圆的并集，底部略收平。Yoyo 字标里的 terracotta 圆盘。 */
    discs: function (p) {
      p = p || {};
      var base = (p.r || 0.82) * R;
      var sep = (p.sep != null ? p.sep : 0.22) * R;
      var flat = p.flat != null ? p.flat : 0.08;
      function hit(cx, th) {
        var c = Math.cos(th);
        var b = -2 * c * cx;
        var disc = b * b - 4 * (cx * cx - base * base);
        if (disc < 0) return 0;
        return (-b + Math.sqrt(disc)) / 2;
      }
      return sampleRadial(function (th) {
        var r = Math.max(hit(-sep, th), hit(sep, th));
        var seat = 1 - flat * Math.pow(Math.max(0, Math.sin(th)), 3);
        return r * seat;
      }, p);
    }
