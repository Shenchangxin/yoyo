...
import { DEFAULT_CANVAS_APPEARANCE } from "@yingce/lib/canvas/agent-appearance";

import type { PublicAppearance } from "@yingce/services/api/appearance";
import { applySkinTheme, DEFAULT_CLASSIC_SKIN, normalizeSkinDefinition } from "@yingce/lib/skin-themes";
...
export const DEFAULT_PUBLIC_APPEARANCE: PublicAppearance = {
    ...
    seoTitle: "Yoyo",
    seoDescription: "Yoyo infinite canvas.",
    seoKeywords: "",
    footerCopyright: `© ${new Date().getFullYear()} Yoyo`,
...
export function applyAppearanceMetadata(appearance: PublicAppearance, targetDocument: Document | undefined = typeof document === "undefined" ? undefined : document) {
    if (typeof window !== "undefined" && (window as Window & { __YOYO_CANVAS_HOST__?: boolean }).__YOYO_CANVAS_HOST__) return;
...
export function brandStudioLabel(appearance: PublicAppearance) {
    if (appearance.brandName === DEFAULT_PUBLIC_APPEARANCE.brandName && appearance.brandSlug === DEFAULT_PUBLIC_APPEARANCE.brandSlug) return "YOYO CANVAS";
