// @ts-nocheck
import type { AiTextMessage } from "@yingce/services/api/image";
import { imageReferenceLabel } from "@yingce/lib/image-reference-prompt";
import { seedanceReferenceLabel } from "@yingce/lib/seedance-video";
import type { ReferenceImage } from "@yingce/types/image";
import type { ReferenceAudio, ReferenceVideo } from "@yingce/types/media";
import { CanvasNodeType, type CanvasConnection, type CanvasGenerationMode, type CanvasNodeData } from "@yingce/types/canvas";
import { getGenerationResourceNodes, getContextResourceNodes, getMentionResourceNodes } from "@yingce/lib/canvas/canvas-resource-references";
import { canvasNodeVideoPreviewUrl, canvasVideoAssetPreviewUrl } from "@yingce/lib/canvas/canvas-media-preview";
import { isNeutralColorGrade, resolveCanvasColorGradeReference } from "@yingce/lib/canvas/canvas-color-grade";
import { getNodeResourceKind } from "@yingce/lib/canvas/node-registry";
import { mediaConversionSourceFingerprint } from "@yingce/lib/media-conversion/contracts";
import { resolveCanvasDrawingReference } from "@yingce/lib/canvas/canvas-drawing-reference";
import { compileCharacterReferencePrompt } from "@yingce/lib/canvas/canvas-character-reference";
import { nodeReferenceImage } from "@yingce/lib/canvas/canvas-project-generation";
import { isCanvasWorkflowProvider } from "@yingce/lib/canvas/canvas-workflow";
import { audioFileExtension } from "@yingce/lib/character-voice-formats";
import type { ModelReferenceLimits } from "@yingce/lib/model-selection";
...
export async function hydrateNodeGenerationContext(context: NodeGenerationContext, projectId: string, domainProjectId?: string, mode?: CanvasGenerationMode, includeCharacterVoiceSamples = false, includeCharacterPrompt = true, referenceLimits?: ModelReferenceLimits) {
    const { imageToDataUrl } = await import("@yingce/services/image-storage");
    ...
    const { getProjectCharacter } = await import("@yingce/services/api/projects");
    const { getResource, resourceFileUrl, resourceIdFromStorageKey, resourceStorageKey } = await import("@yingce/services/api/resources");
