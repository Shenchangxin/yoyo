...
import { DirectorViewport, type DirectorViewportHandle } from "@yingce/components/canvas/director/director-viewport";
import { DirectorViewportDock } from "@yingce/components/canvas/director/director-viewport-dock";
import { DirectorSequencer } from "@yingce/components/canvas/director/director-sequencer";
import { canvasThemes } from "@yingce/lib/canvas-theme";
import { compileDirectorPrompt } from "@yingce/lib/canvas/director/director-prompt-compiler";
import { advanceDirectorPlayhead, resolveDirectorCameraAlignment, resolveDirectorCameraMoveKeyframes, resolveDirectorKeyframeRecord, resolveDirectorObjectTransformEdit, snapDirectorTime } from "@yingce/lib/canvas/director/director-animation-semantics";
import { createDirectorTransaction, installDirectorTerminalListeners, type DirectorTransaction } from "@yingce/lib/canvas/director/director-gesture-transaction";
import { recordDirectorDiagnostic } from "@yingce/lib/canvas/director/director-diagnostics-recorder";
import { DIRECTOR_MODES, directorModeCapabilities, type DirectorModeCapabilities } from "@yingce/lib/canvas/director/director-modes";
import { resolveDirectorPlacement, resolveDirectorPlacementAnchor } from "@yingce/lib/canvas/director/director-placement";
import { isDirectorOutputSnapshotCurrent, shouldReinitializeDirectorSession } from "@yingce/lib/canvas/director/director-session";
import { blocksDirectorShortcut, releaseDirectorFocusAfterPointer, resolveDirectorShortcut, type DirectorShortcutAction } from "@yingce/lib/canvas/director/director-shortcuts";
import { createDirectorActor, createDirectorBillboard, createDirectorCamera, createDirectorLight, createDirectorModel, createDirectorObject, DIRECTOR_ACTOR_COLORS, directorBoneLabel, directorFocalLengthToFov, directorPoseLabel, interpolateDirectorTransform, removeDirectorSceneKeyframe, setDirectorSceneKeyframeEasing, touchDirectorScene, upsertDirectorBoneKeyframe } from "@yingce/lib/canvas/director/director-scene";
import { describeDirectorSaveStatus, resolveDirectorCloseOutcome, shouldBlockDirectorUnload, shouldOfferDirectorDraftRecovery } from "@yingce/lib/canvas/director/director-save-wiring";
import { useDirectorSaveCoordinator } from "@yingce/components/canvas/director/use-director-save-coordinator";
import { uploadMediaFile } from "@yingce/services/file-storage";
import { localSavedRemotePendingMessage, saveRemoteUserDataNow } from "@yingce/services/user-data-sync";
import { useAssetStore, type ModelAsset } from "@yingce/stores/use-asset-store";
import { useDirectorWorkbenchStore } from "@yingce/stores/canvas/use-director-workbench-store";
...
import type { DirectorCamera, DirectorCameraMove, DirectorHumanoidBone, DirectorKeyframeDeleteTarget, DirectorKeyframeEasing, DirectorLight, DirectorObject, DirectorPose, DirectorQuat, DirectorRenderMode, DirectorRig, DirectorScene, DirectorSceneOutput, DirectorShot, DirectorShotSize, DirectorTransform, DirectorVec3 } from "@yingce/types/director";
