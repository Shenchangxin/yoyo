...
import { PageHeader } from "@yingce/components/layout/workspace-page";
import { listRegisteredPlugins } from "@yingce/lib/plugins/plugin-registry";
import "@yingce/lib/plugins/builtin";
import { EAGLE_PLUGIN_ID } from "@yingce/lib/plugins/builtin/eagle";
import { PROMPT_OPTIMIZER_PLUGIN_ID } from "@yingce/lib/plugins/builtin/prompt-optimizer";
import { RUNNINGHUB_PLUGIN_ID } from "@yingce/lib/plugins/builtin/workflows";
import { isOfficialApplicationPluginId } from "@yingce/lib/plugins/official-applications";
import type { PluginManifest, PluginManifestV2, RegisteredPlugin } from "@yingce/lib/plugins/plugin-types";
import { getEagleLibrary, type EagleFolder } from "@yingce/services/api/eagle";
import { fetchPlugins, setUserPluginEnabled, type BackendPlugin, type PluginState } from "@yingce/services/api/plugins";
...
function toRegisteredPlugin(plugin: BackendPlugin): RegisteredPlugin {
    const manifest = { ...plugin.manifest };
    if (manifest.author) manifest.author = debrandYingce(manifest.author);
    if (manifest.documentation) manifest.documentation = debrandYingce(manifest.documentation);
    if (manifest.description) manifest.description = debrandYingce(manifest.description);
