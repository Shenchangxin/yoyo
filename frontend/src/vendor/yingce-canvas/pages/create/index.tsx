...
import type { AssetLibraryPickerItem } from "@yingce/components/assets/asset-library-picker-modal";
import { generationErrorCode, generationErrorMessage } from "@yingce/lib/generation-error";
...
import { continueCreationConversationOnCanvas } from "@yingce/services/creation-canvas-conversation";
import { useExternalAssetSources } from "@yingce/hooks/use-external-asset-sources";
import { modelCapabilityConfigFor, normalizeImageValue, normalizeVideoValue, videoDurationAllowed, videoDurationOptions } from "@yingce/lib/model-capabilities";
import { inferVideoOperation, modelGroupReferenceLimits, resolveCompatibleModel, mergedImageCapabilityConfig, type ModelRequirements } from "@yingce/lib/model-selection";
import type { BackendGenerationResult } from "@yingce/services/api/generation-task";
import type { Skill } from "@yingce/services/api/skills";
import type { GenerationTask } from "@yingce/services/api/task-center";
import { loadCreationConversations, pendingCreationTaskIds, removeCreationConversationSnapshot, saveCreationConversations, updateCreationConversationSnapshot } from "@yingce/services/creation-conversation-store";
import { resolveModelChannel, selectableModelsByCapability, useConfigStore, useEffectiveConfig } from "@yingce/stores/use-config-store";
import { useCreationPreferencesStore } from "@yingce/stores/use-creation-preferences-store";
import { useAssetStore, type Asset } from "@yingce/stores/use-asset-store";
...
import type { PromptOptimizerProvider } from "@yingce/lib/plugins/plugin-types";
import { promptOptimizerPlugin, PROMPT_OPTIMIZER_PLUGIN_ID } from "@yingce/lib/plugins/builtin/prompt-optimizer";
import { createPluginHostContext } from "@yingce/services/plugin-host";
...
const AssetLibraryPickerModal = lazy(() => import("@yingce/components/assets/asset-library-picker-modal").then((module) => ({ default: module.AssetLibraryPickerModal })));
...
export default function CreatePage() {
    ...
    const loadAddedSkills = useCallback(() => {
        ...
        void import("@yingce/services/api/skills")
