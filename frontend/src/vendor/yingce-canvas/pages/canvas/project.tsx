// @ts-nocheck
import { CanvasWorkspacePanel } from "@yingce/components/canvas/canvas-workspace-panel";
import { isCanvasNodeGenerating } from "@yingce/lib/canvas/canvas-node-task-state";
import { createCanvasStateWriter } from "@yingce/lib/canvas/canvas-editor-state";
import { canCancelGenerationTask } from "@yingce/lib/generation-task-display";
...
import { createCanvasGenerationLiveProjectAdapter, registerCanvasGenerationLiveProject } from "@yingce/services/canvas-generation-consumer";
import { getActiveUserScope } from "@yingce/lib/user-scope";
import { getResourceAccess, resolveResourceAccessURL, resourceFileUrl, resourceIdFromStorageKey, syncResourceToArkPrivateAsset } from "@yingce/services/api/resources";
import { uploadImage } from "@yingce/services/image-storage";
import { imageMetadata } from "@yingce/lib/canvas/canvas-generation-task-sync";
import { isCanvasImageSourceNode } from "@yingce/lib/canvas/canvas-image-source";
import { getCachedResourceBlob } from "@yingce/services/resource-blob-cache";
import copyToClipboard from "copy-to-clipboard";
import { nanoid } from "nanoid";
import { canvasAppearanceBaseTheme, canvasAppearanceForTheme, DEFAULT_CANVAS_BACKGROUND_MODE, normalizeCanvasAppearance, resolveCanvasAppearance, writeCanvasAppearanceDefault, type CanvasAppearance } from "@yingce/lib/canvas/canvas-appearance";
import { canvasThemes, type CanvasBackgroundMode } from "@yingce/lib/canvas-theme";
import { persistCanvasMediaPerformanceMode, readCanvasMediaPerformanceMode } from "@yingce/lib/canvas/canvas-performance-mode";
import { filterCanvasDisplayConnections, persistCanvasHideNodeConnections, readCanvasHideNodeConnections } from "@yingce/lib/canvas/canvas-connection-visibility";
import { summarizeCanvasContext } from "@yingce/lib/canvas/canvas-context-summary";
import { refreshCanvasCharacterReferenceNodes } from "@yingce/lib/canvas/canvas-character-reference";
import { useAssetStore } from "@yingce/stores/use-asset-store";
import { flushCanvasStorePersistence } from "@yingce/stores/canvas/use-canvas-store";
import { ensureCanvasNodeAsset } from "@yingce/services/project-asset-sync";
import { useCanvasThemeStore, useCanvasThemeScope } from "@yingce/stores/canvas/use-canvas-theme-store";
...
import { CanvasCloudAgentPanel } from "@yingce/components/canvas/canvas-cloud-agent-panel";
import { CanvasActiveTaskPanel } from "@yingce/components/canvas/canvas-active-task-panel";
import { CanvasProjectSidebar } from "@yingce/components/canvas/canvas-project-sidebar";
import { CanvasProjectAssetModal } from "@yingce/components/canvas/canvas-project-asset-modal";
import { CanvasCharacterReferenceNodeContent } from "@yingce/components/canvas/canvas-character-reference-node";
import { CanvasCharacterReferenceModal } from "@yingce/components/canvas/canvas-character-reference-modal";
...
import { createStyleProfileSnapshot, resolveStyleProfile, serializeStyleProfile } from "@yingce/lib/canvas/style-profile";
import { CanvasNodeToolbar, CanvasNodeInfoModal } from "@yingce/components/canvas/canvas-node-toolbar";
import { CanvasSubtitleDialog } from "@yingce/components/canvas/canvas-subtitle-dialog";
import { CanvasVideoFrameDialog } from "@yingce/components/canvas/canvas-video-frame-dialog";
import { CanvasVideoSegmentDialog } from "@yingce/components/canvas/canvas-video-segment-dialog";
import { CanvasTimelineDialog } from "@yingce/components/canvas/canvas-timeline-dialog";
import { syncNodeSubtitlesToTimeline } from "@yingce/lib/timeline/timeline-build";
import { CanvasNodeAnglePanel } from "@yingce/components/canvas/canvas-node-angle-dialog";
import { CanvasNodeLightingPanel } from "@yingce/components/canvas/canvas-node-lighting-dialog";
import { CanvasTextEditorModal } from "@yingce/components/canvas/canvas-text-editor-modal";
import { CanvasNodeSearchModal } from "@yingce/components/canvas/canvas-node-search-modal";
import { CanvasStylePickerModal } from "@yingce/components/canvas/canvas-style-picker-modal";
import { CanvasDirectorTemplateModal } from "@yingce/components/canvas/director/canvas-director-template-modal";
import { CanvasFileDropOverlay } from "@yingce/components/canvas/canvas-file-drop-overlay";
import { CanvasUploadModal } from "@yingce/components/canvas/canvas-upload-modal";
import { CanvasPanoramaConfigModal } from "@yingce/components/canvas/canvas-panorama-config-modal";
import { InfiniteCanvas } from "@yingce/components/canvas/infinite-canvas";
import { Minimap } from "@yingce/components/canvas/canvas-mini-map";
import { CanvasNodePromptPanel, type CanvasNodeGenerationMode } from "@yingce/components/canvas/canvas-node-prompt-panel";
import { handleListGenerate } from "./list-mode-generator";
import { CanvasToolbar } from "@yingce/components/canvas/canvas-toolbar";
import { useCanvasCreateCommands } from "@yingce/components/canvas/use-canvas-create-commands";
import { AssetPickerModal } from "@yingce/components/canvas/asset-picker-modal";
import { getProject } from "@yingce/services/api/projects";
import { CanvasZoomControls } from "@yingce/components/canvas/canvas-zoom-controls";
import { CanvasShareModal } from "@yingce/components/canvas/canvas-share-modal";
import { CanvasScriptEditor, CanvasScriptNodeContent } from "@yingce/components/canvas/canvas-script-node";
import { CanvasBatchTableNodeContent } from "@yingce/components/canvas/canvas-batch-table-node";
import { BatchGenerationSettingsDialog } from "@yingce/components/canvas/batch-generation-settings-dialog";
import { batchReferenceColumns, promoteLegacyBatchTableSize } from "@yingce/lib/canvas/canvas-batch-table";
import { STORYBOARD_HEADER_HEIGHT, STORYBOARD_ROW_HEIGHT, storyboardMinNodeHeight, storyboardTableHeight } from "@yingce/lib/canvas/canvas-storyboard-layout";
import { CanvasDirectorNodePanel } from "@yingce/components/canvas/director/canvas-director-node-panel";
import { CanvasVersionCompareModal } from "@yingce/components/canvas/canvas-version-compare-modal";
import { useFocusMode } from "@yingce/hooks/use-focus-mode";
import { connectCanvasTextMention } from "@yingce/lib/canvas/canvas-text-mention";
import { writeCanvasNodePrompt } from "@yingce/lib/canvas/canvas-node-prompt";
import {
...
} from "@yingce/lib/canvas/canvas-resource-references";
import { CanvasConnectionCreateMenu, CanvasNodePanelOverlay, type PendingConnectionCreate } from "@yingce/components/canvas/canvas-workspace-overlays";
import { CanvasOverlayLayerContainer, CanvasOverlayLayerProvider } from "@yingce/components/canvas/canvas-overlay-layer";
import { CanvasLeaferGraphicsLayer } from "@yingce/components/canvas/canvas-leafer-graphics-layer";
import { CanvasFreeformEmptyState, CanvasLinkedProjectEmptyState, CanvasShortDramaEmptyState, CanvasShortDramaGuide, CanvasStoryInputNodeContent, CanvasStylePlaceholderNodeContent } from "@yingce/components/canvas/canvas-short-drama-entry";
import { resolveCanvasEmptyStateKind } from "@yingce/lib/canvas/canvas-starter";
import { failedImageBatchChildren, markImageBatchRetrying, reconcileImageBatchRoot, restoreUnsubmittedImageBatchChild } from "@yingce/lib/canvas/canvas-image-batch-retry";
import { cinematicStoryboardColumns, createCanvasNode, getInputSummary, isHiddenBatchChild } from "@yingce/lib/canvas/canvas-project-domain";
import { bindStoryboardKeyframes, storyboardKeyframeTime, storyboardRowsFromBatchTable } from "@yingce/lib/canvas/canvas-batch-storyboard";
import { stampCanvasNodeChanges, updateCanvasNode, updateCanvasNodes } from "@yingce/lib/canvas/canvas-node-timestamps";
import { canvasAssetHandoffAttempt, finalizeCanvasAssetHandoff, uninsertedCanvasAssetHandoffPayloads } from "@yingce/lib/canvas/canvas-asset-handoff";
import { batchSourceRestriction } from "@yingce/lib/canvas/canvas-batch-connection";
import { deriveStoryboardPipelineProgress } from "@yingce/lib/canvas/canvas-storyboard-progress";
import { CanvasOperationChangeToast, CanvasMergeStatusToast, CanvasUploadStatusToast } from "./canvas-project-feedback";
import { backendProviderConfig, getGenerationCount } from "@yingce/lib/canvas/canvas-project-generation";
import { cancelGenerationTask } from "@yingce/services/api/task-center";
...
import { CanvasFocusModeBar } from "@yingce/components/canvas/canvas-focus-mode-bar";
...
import { CanvasNodeActionContext, type CanvasNodeActionContextValue } from "@yingce/components/canvas/canvas-node-action-context";
import { bringCanvasNodeToFront, type CanvasNodeStackOrder } from "@yingce/lib/canvas/canvas-node-stack-order";
import { AiArtCritiqueModal } from "@yingce/components/canvas/art-critique/ai-art-critique-modal";
import { CanvasNodeGraphContext, type CanvasNodeGraphContextValue } from "@yingce/components/canvas/canvas-node-graph-context";
import { CanvasRefreshShell } from "./canvas-refresh-shell";
import { queryGenerationTask } from "@yingce/services/api/task-center";
import type { CanvasImageEmotionPayload } from "@yingce/components/canvas/canvas-node-emotion-panel";
import { CanvasEmotionWorkspace } from "@yingce/components/canvas/canvas-emotion-workspace";
import { removeCanvasDrawing } from "@yingce/lib/canvas/canvas-drawing-storage";
...
} from "@yingce/types/canvas";
import type { ReferenceImage } from "@yingce/types/image";
import { ART_CRITIQUE_NODE_TYPE } from "@yingce/lib/art-critique/contracts";

const CanvasDirectorWorkbench = lazy(() => import("@yingce/components/canvas/director/canvas-director-workbench").then((module) => ({ default: module.CanvasDirectorWorkbench })));
const CanvasDrawingEditorModal = lazy(() => import("@yingce/components/canvas/canvas-drawing-editor-modal").then((module) => ({ default: module.CanvasDrawingEditorModal })));
...
function InfiniteCanvasPage() {
    ...
    const cancelCanvasTask = useCallback(
        (task: import("@yingce/services/api/task-center").GenerationTask) => {
