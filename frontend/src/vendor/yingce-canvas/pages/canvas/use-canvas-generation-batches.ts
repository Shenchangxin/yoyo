...
import { generationBatchStatus, isGenerationCostUncertainError } from "@yingce/lib/canvas/canvas-generation-batch";
import { buildGenerationConfig, createGenerationRetryContext, generationTaskMetadata, resetGenerationTaskMetadata } from "@yingce/lib/canvas/canvas-project-generation";
import { unchangedModeratedPrompt } from "@yingce/lib/generation-error";
import { listGenerationTasks } from "@yingce/services/api/task-center";
...
import type { CanvasGenerationBatch, CanvasGenerationBatchItem, CanvasGenerationBatchMode, CanvasNodeData } from "@yingce/types/canvas";
...
export function useCanvasGenerationBatches({ projectId, projectLoaded, nodes, nodesRef, setNodes, handleGenerateNode }: UseCanvasGenerationBatchesOptions) {
    ...
    const stopRemainingBatchItems = useCallback(
        (sourceNodeId: string, batchId: string) => {
            ...
            if (!batch) return;
            const nodeById = new Map(nodesRef.current.map((node) => [node.id, node]));
            // 只允许停止还在本地等待队列中的项目。进入 submitting 后请求可能已经被引擎接收，
