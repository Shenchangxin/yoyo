...
import { http, apiBaseURL, ApiError } from "@yingce/services/api/request";
...
export function resourceFileUrl(id: string) {
    const media = typeof window !== "undefined" ? String((window as Window & { __YOYO_MEDIA_BASE__?: string }).__YOYO_MEDIA_BASE__ || "").replace(/\/+$/, "") : "";
    if (media) return `${media}/resource/${encodeURIComponent(id)}`;
