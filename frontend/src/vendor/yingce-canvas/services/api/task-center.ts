...
import { http, apiBaseURL, type BackendEnvelope } from "@yingce/services/api/request";
import { consumeTaskTextStream, createTaskTextStreamParser, type TaskTextStreamEvent } from "@yingce/services/api/task-text-stream";
import { recordDiagnosticEvent } from "@yingce/services/diagnostics/client-diagnostics";

export type { BackendEnvelope } from "@yingce/services/api/request";
...
async function waitForGenerationTaskTextEvents(id: string, options: WaitForGenerationTaskOptions) {
    ...
    while (Date.now() - startedAt < timeoutMs) {
        if (options.signal?.aborted) throw new DOMException("Aborted", "AbortError");
        const replay = await queryTaskTextReplay(id, lastEventId);
        for (const delta of replay.deltas || []) {
            const sequence = delta.sequence || 0;
        ...
        if (replay.textDraft && replay.textDraft.length >= fullText.length) {
            fullText = replay.textDraft;
            options.onTextDelta?.(fullText);
        }
        lastTask = await queryGenerationTask(id, { signal: options.signal });
        if (fullText) lastTask = { ...lastTask, textDraft: fullText };
        options.onTaskUpdate?.(lastTask);
        if (lastTask.status === "succeeded") {
            if (replay.finalText) options.onTextDelta?.(replay.finalText);
            return lastTask;
        }
        if (lastTask.status === "failed" || lastTask.status === "cancelled") {
            throw new TaskTextStreamFatalError(lastTask.error ? generationErrorMessage(lastTask.error) : `任务${lastTask.status === "cancelled" ? "已取消" : "失败"}`);
        }
        await delay(intervalMs, options.signal);
    }
