...
async function encodeCanvasBody(data: unknown) {
    const out: { body: unknown; file_b64: string; file_name: string; file_type: string; chunk_b64: string } = {
        body: data ?? {},
        file_b64: "",
        file_name: "",
        file_type: "",
    ...
    if (typeof FormData !== "undefined" && data instanceof FormData) {
        ...
        for (const [key, value] of data.entries()) {
            if (typeof Blob !== "undefined" && value instanceof Blob) {
                const b64 = await blobToB64(value);
                if (key === "chunk") out.chunk_b64 = b64;
                else {
                    out.file_b64 = b64;
                    out.file_name = value instanceof File ? value.name : "";
                    out.file_type = value.type;
                }
                body[key] = value instanceof File ? value.name : key;
            } else {
                body[key] = value;
            }
        }
    ...
    if (typeof Blob !== "undefined" && data instanceof Blob) {
        out.chunk_b64 = await blobToB64(data);
        out.body = {};
        return out;
    }
    if (typeof ArrayBuffer !== "undefined" && data instanceof ArrayBuffer) {
        out.chunk_b64 = await blobToB64(new Blob([data]));
        out.body = {};
    }
    return out;
}

function canvasPath(url: string) {
    return String(url || "")
        .replace(/^https?:\/\/[^/]+/i, "")
        .replace(/^\/api\/?/, "")
        .replace(/^\//, "")
...
async function send<T>(method: string, url: string, data?: unknown, config?: HttpRequestConfig) {
    const { videoCall } = await import("@/lib/client");
    const extra = await encodeCanvasBody(data);
    try {
        const env = await videoCall("canvas.http", {
            ...
            chunk_b64: extra.chunk_b64,
        });
        const envelope = (env && typeof env === "object" ? env : { code: 0, data: env, msg: "" }) as BackendEnvelope<T>;
        if (envelope.code && envelope.code !== 0) {
            throw new ApiError(envelope.msg || "请求失败", {
                status: envelope.code,
                code: envelope.code,
                reason: envelope.reason,
                details: envelope.details,
                retryable: isRetryableStatus(envelope.code),
            });
        }
