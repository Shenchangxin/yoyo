// @ts-nocheck
import { getFeatureAvailability, type AuthSessionPayload } from "@yingce/services/api/auth";
import { getModelCatalog, type CapabilitySpec, type ModelCatalogResponse, type OptionConstraint, type PublicChannelCatalog } from "@yingce/services/api/logical-models";
import { localForageStorage } from "@yingce/lib/localforage-storage";
import { appQueryClient } from "@yingce/lib/query-client";
import { scopedLocalStorage, setActiveUserScope, withUserScopedPersistenceSuppressed } from "@yingce/lib/user-scope";
import { CANVAS_STORE_KEY, flushCanvasStorePersistence, useCanvasStore, withCanvasStorePersistenceSuppressed } from "@yingce/stores/canvas/use-canvas-store";
import { CANVAS_HISTORY_STORE_KEY, useCanvasHistoryStore } from "@yingce/stores/canvas/use-canvas-history-store";
import { ASSET_STORE_KEY, flushAssetStorePersistence, useAssetStore, withAssetStorePersistenceSuppressed } from "@yingce/stores/use-asset-store";
import { CONFIG_STORE_KEY, defaultConfig, normalizeConfigSnapshot, useConfigStore, type ModelCapability, type ModelChannel } from "@yingce/stores/use-config-store";
import { CREATION_PREFERENCES_STORE_KEY, useCreationPreferencesStore } from "@yingce/stores/use-creation-preferences-store";
import { defaultModelCapabilityConfig, STANDARD_IMAGE_SIZE_VALUES, type ModelCapabilityConfig } from "@yingce/lib/model-capabilities";
import { imageSizeConfigWithPresets } from "@yingce/lib/image-size-presets";
import { useUserStore } from "@yingce/stores/use-user-store";
import { PLUGIN_STORE_KEY, usePluginStore } from "@yingce/stores/use-plugin-store";
import { initializeRemoteUserDataSession, installRemoteUserDataAutoSync, resetRemoteUserDataSync, withRemoteUserDataSyncExclusive } from "@yingce/services/user-data-sync";
import { clearResourceAccessCache } from "@yingce/services/api/resources";
import { clearResourceBlobCache } from "@yingce/services/resource-blob-cache";
import { withGenerationConsumersPaused } from "@yingce/services/generation-consumer-lifecycle";
