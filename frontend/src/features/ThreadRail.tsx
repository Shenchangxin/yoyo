...
const LABS: { id: Lab; label: string; hint: string; icon: typeof Shield }[] = [
  { id: "harbor", label: "Harbor", hint: "Eval before promote", icon: Shield },
  { id: "evolve", label: "Evolve", hint: "ACE / DGM cycle", icon: FlaskConical },
  { id: "harness", label: "Harness", hint: "Refs and checkout", icon: GitBranch },
];
...
export function ThreadRail(props: {
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col overflow-hidden bg-sidebar">
      ...
      <nav aria-label="Chats" className="min-h-0 flex-1 overflow-auto px-2 pb-2">
        {list.length === 0 ? <div className="px-3 py-6 text-xs text-muted">No chats yet</div> : null}
        {list.map((t) => {
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="block truncate text-[11px] text-muted">{run ? "Running" : t.workspace ? short(t.workspace) : "Idle"}</span>
              </span>
            </button>
          );
        })}
      ...
      <div className="border-t border-border p-2">
        <div className="px-3 pb-1 text-[11px] text-muted">Labs</div>
        {LABS.map((l) => {
          const Icon = l.icon;
          return (
            <button
